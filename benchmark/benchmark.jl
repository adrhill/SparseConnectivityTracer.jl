...
for S in SET_TYPES
    ...
    for sparsity in (0.01, 0.05, 0.1, 0.25, 0.5)
        ...
        J_global["sparse_matmul"]["$sparsity"][setname] = @benchmarkable jacobian_sparsity(
            $f, $x, $method
        )
    end
    ...
    for N in (6, 24, 100)
        ...
        x = rand(N, N, 2)
        y = similar(x)
        J_global["brusselator"]["$N"][setname] = @benchmarkable jacobian_sparsity(
            $f!, $y, $x, $method
        )
        ...
        J_global["brusselator_ode_solve"]["$N"][setname] = @benchmarkable jacobian_sparsity(
    ...
    ## Deep Learning
    for N in (28, 128)
        J_global["conv"]["$N"][setname] = @benchmarkable jacobian_sparsity(
